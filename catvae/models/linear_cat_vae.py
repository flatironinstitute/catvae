...
class LinearCatVAE(nn.Module):
    ...
    def __init__(self, input_dim : int, hidden_dim : int,
        ...
        if encoder_depth > 1:
            self.final_encoder = nn.Linear(
                hidden_dim, self.input_dim, bias=False)
            ...
            layers.append(self.final_encoder)
            self.encoder = nn.Sequential(*layers)
        else:
            self.encoder = nn.Linear(hidden_dim, self.input_dim, bias=False)
